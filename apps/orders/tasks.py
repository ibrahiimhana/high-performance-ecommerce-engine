"""Celery tasks: async dispatch from checkout + nightly sales rollup."""
...
def rollup_daily_sales(target_date_str: str | None = None) -> dict:
    """Aggregate the previous day's sales. Streams through OrderItems
    in chunks so peak memory stays O(CHUNK_SIZE) regardless of volume."""
    if target_date_str:
    ...
    chunk: list[tuple] = []
    # iterator() uses a server-side cursor — rows stream from Postgres
